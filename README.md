...
Email helper for CapstoneJS and Node.js Applications. Makes it easy to send dynamic emails from templates, to one or multiple recipients. This project was forked from [keystone-email](https://github.com/keystonejs/keystone-email.git) and fixed.
...
* Understands Capstone User models, making it easy to send emails to the results of a query
...
We're moving the built-in email functionality out of CapstoneJS core (`capstone.Email`) into its own package, so that it's not bundled by default and can be updated independently.
...
```
var Email = require('capstone-email')
...
The `from` option can be a String (email address), or Object containing `name` and `email`. In the object form, `name` can also be an object containing `first` and `last` (which will be concatenated with a space). This simplifies usage with `User` models in CapstoneJS. For example:

```js
{ from: 'user@capstonejs.com' };
{ from: { email: 'user@capstonejs.com', name: 'Jed Watson' } };
{ from: { email: 'user@capstonejs.com', name: { first: 'Jed', last: 'Watson' } } };
```
...
```js
// single recipient
{ to: 'user@capstonejs.com' }
{ to: { email: 'user@capstonejs.com', name: 'Jed Watson' } }
{ to: {
	email: 'user@capstonejs.com',
	name: { first: 'Jed', last: 'Watson' },
	vars: { role: 'Developer' }
} }
// multiple recipients
{ to: [
	{ email: 'user@capstonejs.com', name: 'Jed Watson', vars: { home: 'Sydney' } }
	{ email: 'user@capstonejs.com', name: 'Max Stoiber', vars: { home: 'Vienna' } }
] }
```

## Breaking changes from `capstone.Email`
...
Capstone would previously automatically tag emails with the sent date and template name. That's pretty opinionated and of questionable usefulness, if you still want this behaviour it's easy to add tags yourself in the `send()` options.

### Path changes

You could previously use a directory as the template name, and Capstone.Email would look for an `email.{ext}` file (where ext is your template language's default extension). This is no longer supported, please provide the full path to your email template.

### To, From and Contents validation

Capstone would previous return errors if the subject, contents, recipient(s) or the sender address were invalid. This is no longer handled by the library, please make sure you validate these options before sending the emails. If the transport validates these options, errors will be passed directly to the callback.
